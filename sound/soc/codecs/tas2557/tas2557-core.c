...
#else
	unsigned char *pDevABuf = NULL, *pDevBBuf = NULL;
#endif
/* HTC_AUD_END */
...
/* HTC_AUD_START: remve clear firmware due to not reload back */
#if 0
...
#else
					hrtimer_start(&pTAS2557->mtimer,
						ns_to_ktime((u64)100 * NSEC_PER_MSEC), HRTIMER_MODE_REL);
#endif
/* HTC_AUD_END */
				}
			}
...
/* HTC_AUD_START Fix Klockwork */
	if (nPosition < 0) {
		dev_err(pTAS2557->dev, "Firmware: parse pll data failed");
		return -EINVAL;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START Fix Klockwork */
	if (nPosition < 0) {
		dev_err(pTAS2557->dev, "Firmware: parse program data failed");
		return -EINVAL;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START Fix Klockwork */
	if (nPosition < 0) {
		dev_err(pTAS2557->dev, "Firmware: parse configuration data failed");
		return -EINVAL;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START: calibration table without version flag */
#if 0
	if (nSize > 64)
	...
	if (nSize > 64) {
		nPosition = fw_parse_calibration_data(pTAS2557, pFirmware, pData);
		if (nPosition < 0) {
			dev_err(pTAS2557->dev, "Firmware: parse calibration data failed");
			return -EINVAL;
		}

		pData += nPosition;
		if (nPosition && *pData == 0) /* version 0 not use */
			return -EINVAL;
	}
#endif
/* HTC_AUD_END */
...
/* HTC_AUD_START */
	if (config == -1) {
		dev_dbg(pTAS2557->dev, "default Configuration %d \n", config);
		goto end;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
	...
	if (offset != NULL) {
		pr_info("TAS2557 cali_data load success from dts\n");
		p_data = (unsigned char *) of_get_property(offset, AUDIO_DATA, &p_size);
	} else {
		pr_err("TAS2557 cali_data load fail on dts\n");
	}
/* HTC_AUD_END */
	...
	if (nFile >= 0) {
		nSize = sys_read(nFile, pBuffer, 1024);
		sys_close(nFile);
	} else {
		dev_dbg(pTAS2557->dev, "TAS2557 cannot open calibration file: %s\n",
			pFileName);
/* HTC_AUD_START - use dts calibration data if file could not open */
		if (p_data && p_size != 0) {
			use_dts = true;
			nSize = p_size;
			pr_info("TAS2557 use cali_data from dts\n");
			memcpy(pBuffer, p_data, 1024);
		}
/* HTC_AUD_END */
	}
...
/* HTC_AUD_START */
	if (nResult && use_dts) {
		tas2557_clear_firmware(pTAS2557->mpCalFirmware);
		dev_err(pTAS2557->dev, "TAS2557 calibration: default cali due to old tool\n");
		return nResult;
	}
/* HTC_AUD_END */
...
#else
	pTAS2557->mnLeftChlGpioRst = of_get_named_gpio(np, "ti,cdc-reset-gpio", 0);
#endif
/* HTC_AUD_END  */
